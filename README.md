...
[![Coverage
Status](https://coveralls.io/repos/github/pbulsink/kinvicalc/badge.svg?branch=main)](https://coveralls.io/github/pbulsink/kinvicalc?branch=main)
<!-- badges: end -->
...
`evaluate_determinability()` for what happens (and how it is reported)
when it does not.
...
Do not `source()` or run `R/app.R` directly from a package checkout —
always launch via `run_app()` (or, for local development, the thin
delegator at `inst/shiny/app.R`), so that package-internal helpers
resolve correctly.
