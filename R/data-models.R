...
validate_viscometer_record <- function(viscometer) {
  if (is.null(viscometer) || !is.data.frame(viscometer)) {
    stop("`viscometer` must be a data frame or tibble.", call. = FALSE)
  }
  ...
  if (length(missing_cols) > 0) {
    stop(
      sprintf(
        "Viscometer record is missing required columns: %s",
  ...
  assert_string(viscometer$viscometer_id[1], "viscometer_id")
  if (!is_valid_viscometer_id(viscometer$viscometer_id[1])) {
    stop("`viscometer_id` must match the pattern `###-#####`.", call. = FALSE)
  }

  assert_scalar_numeric(viscometer$viscometer_size[1], "viscometer_size")
  assert_string(viscometer$serial_number[1], "serial_number")
  assert_date(viscometer$calibration_date[1], "calibration_date")
  assert_string(viscometer$status[1], "status")
  ...
  for (field in required_factors) {
    if (is.na(viscometer[[field]][1]) || !is.numeric(viscometer[[field]][1])) {
      stop(
        sprintf(
          "Viscometer record must include a numeric value for '%s'.",
          field
        ),
        call. = FALSE
      )
...
validate_sample_type_rule <- function(rule) {
  if (is.null(rule) || !is.data.frame(rule)) {
    stop("`rule` must be a data frame or tibble.", call. = FALSE)
  }
  ...
  if (length(missing_cols) > 0) {
    stop(
      sprintf(
        "Sample type rule is missing required columns: %s",
        paste(missing_cols, collapse = ", ")
      ),
      call. = FALSE
    )
  ...
  assert_string(rule$sample_type[1], "sample_type")
  if (!rule$metric[1] %in% precision_metrics) {
    stop(
      sprintf(
        "`metric` must be one of: %s.",
        paste(precision_metrics, collapse = ", ")
      ),
      call. = FALSE
    )
  }
  assert_scalar_numeric(rule$temp_min_c[1], "temp_min_c")
  assert_scalar_numeric(rule$temp_max_c[1], "temp_max_c")
  if (rule$temp_min_c[1] > rule$temp_max_c[1]) {
    stop(
      "`temp_min_c` must be less than or equal to `temp_max_c`.",
      call. = FALSE
    )
  }
  assert_scalar_numeric(rule$coefficient_a[1], "coefficient_a")
  assert_scalar_numeric(rule$exponent_b[1], "exponent_b")
  if ("offset" %in% names(rule) && !is.na(rule$offset[1])) {
    assert_scalar_numeric(rule$offset[1], "offset")
  }
...
#' into a single reported result. If a flow time is below the minimum, a
#' kinetic energy correction (D446-24 8.2) is applied automatically; supply
#' `kinetic_energy_factor` in that case.
#'
...
#' @param kinetic_energy_factor Kinetic energy factor `E` (mm2*s), required
#'   only if either flow time is below `min_flow_time_s`.
#' @param min_flow_time_s Minimum flow time, s, before a kinetic energy
#'   correction is required. Defaults to 200 s (ASTM D445-26 6.1.2/10.2);
#'   override with the viscometer/size-specific minimum from Specifications
#'   D446 when it is higher.
...
build_sample_result <- function(
  ...
  determination_1 <- calculate_kinematic_viscosity(
    time_1,
    factor,
    kinetic_energy_factor = kinetic_energy_factor,
    min_flow_time_s = min_flow_time_s
  )
  ...
  result <- list(
    ...
    time_2 = time_2,
    factor = factor,
    kinetic_energy_correction_applied = determination_1$kinetic_energy_correction_applied ||
      determination_2$kinetic_energy_correction_applied,
    kinematic_viscosity_1_cSt = determination_1$viscosity,
    kinematic_viscosity_2_cSt = determination_2$viscosity,
    kinematic_viscosity_cSt = determinability$average_viscosity,
    determinability_difference = determinability$difference,
    determinability_result = determinability$result,
    determinability_limit = determinability$limit,
    operator = operator,
    notes = notes,
    created_at = Sys.time()
  )
  ...
  if (!determinability$passed) {
    warning(
      sprintf(
        paste(
          "Determinability check failed for viscometer '%s' (difference",
          "%s exceeds limit %s). Per ASTM D445-26 11.2.3/12.4.1, repeat the",
          "flow time measurements after cleaning and drying the viscometer",
          "before reporting a result."
        ),
        viscometer_id,
        format_significant(determinability$difference),
        format_significant(determinability$limit)
      ),
      call. = FALSE
    )
  }
